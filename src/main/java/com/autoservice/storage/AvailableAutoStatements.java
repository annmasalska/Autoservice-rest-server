...
import com.autoservice.storage.mapping.AvailableAutoKey;
...
public class AvailableAutoStatements extends TableStorage {
    ...
    public void readByKey() throws ConnectionException {


        AvailableAutoKey key = new AvailableAutoKey("MMF");
        ...
        OperationResult<ColumnList<String>> res = keyspace.prepareQuery(CF_AVAILABLEAUTO)
                .getRow("aaza")
                        //autoPaginate(true)
                .execute();
        ColumnList<String> columns = res.getResult();
        for (Column<String> c : res.getResult()) {
            System.out.println(c.getName());
        }
        // for (Column<String> column : res) {
        //   System.out.println( columns.getColumnByName("logo").getStringValue());

        //System.out.println("axazaa");
         /*   Map<String, String> map = res.getValue("modelsAuction",
                    new MapSerializer<String, String>(UTF8Type.instance, UTF8Type.instance)
                    , new HashMap<String, String>());
